...
Implement the actionable feedback in the 2026-09-26 Excalidraw note in the Android client and publish the matching debug prerelease. Simplify and reorganize the route, home, profile, and settings journeys while preserving tunnel/subscription contracts and requiring explicit user action for account, key, and update operations.
...
- Publish an official debug prerelease with a versionCode above the current public release and attach a verified ARM64 APK. Describe backend-dependent behavior and validation limits accurately.
